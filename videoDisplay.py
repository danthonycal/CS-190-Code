...
def videoWrite():
	...
	if args.mode == -1:
		...
		res_paths = glob.glob(f"{os.getcwd()}\\videos\\frames\\darknet-result-json\\processed\\*.JSON")
	else:
	...
	for path_idx in range(len(res_paths)):
		...
		with open(res_paths[path_idx], 'r') as f:
			...
			while True:
				...
				frame = cv2.resize(frame, None, fx=0.6, fy=0.6, interpolation=cv2.INTER_LANCZOS4)
